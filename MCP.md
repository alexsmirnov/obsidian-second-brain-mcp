...
Flag Control:
  ...
  --pup: Force Playwright→browser | --all-mcp: Enable all | --no-mcp: Disable all
...
Context Triggers (flags override):
  ...
  UI requests → Magic(builder/refiner) → Playwright(test)
...
UI Generation: Magic builder w/ user requirements+project context → Magic refiner if needed → integrate component → Playwright validation
Ex: "Dashboard w/ charts" → Magic builder("dashboard","charts") → edit files integrate → Playwright screenshot
```
...
```yaml
Persona Preferences:
  architect: Sequential(design)+C7(patterns)+avoid Magic | frontend: Magic(UI)+Playwright(test)+C7(React/Vue docs)
  backend: C7(API docs)+Sequential(scale analysis)+avoid Magic | analyzer: Sequential(root cause) primary+C7(solutions) secondary
  security: Sequential(threats)+C7(security patterns)+Playwright(test) | mentor: C7(learning)+Sequential(explanations)+avoid Magic
  refactorer: Sequential(analysis)+C7(patterns)+avoid Magic/Playwright | perf: Sequential(bottlenecks)+Playwright(metrics)+C7(optimization)
  qa: Playwright(testing)+Sequential(edge cases)+C7(testing frameworks)
...
Context Share: Sequential→feeds C7 topic selection | C7 docs→inform Magic generation | Magic→tested w/ Playwright | All cached
